...
pub mod pallet {
	...
	#[pallet::storage]
	#[pallet::getter(fn number_u32)]
	pub type NumberU32<T: Config> = StorageMap<_, Blake2_128Concat, T::AccountId, u32, ValueQuery>;
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn update_number(origin: OriginFor<T>, number: u32) -> DispatchResult {
			let who = ensure_signed(origin)?;
			<NumberU32<T>>::mutate(&who, |n| *n = number);
			Self::deposit_event(Event::NumberStored(number, who));
			Ok(())
		}
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
		pub fn sum_number(origin: OriginFor<T>, number: u32) -> DispatchResult {
			let who = ensure_signed(origin)?;
			<NumberU32<T>>::mutate(&who, |n| *n += number);
			Self::deposit_event(Event::NumberStored(number, who));
			Ok(())
