...
use sp_std::vec::Vec;
pub type Id = u32;
use frame_support::dispatch::{fmt,fmt::Debug};
use frame_support::sp_runtime::{ArithmeticError, SaturatedConversion,traits::Hash};
use frame_support::traits::{Currency, Randomness, ReservableCurrency, Time};
...
pub mod pallet {
	...
	pub(super) type TotalBalance<T: Config> =
		StorageMap<_, Blake2_128Concat, T::AccountId, u128, ValueQuery>;

	#[pallet::storage]
	#[pallet::getter(fn total_kitty)]
	pub(super) type TotalKitty<T: Config> =
	...
	impl<T: Config> GenesisBuild<T> for GenesisConfig<T> {
		fn build(&self) {
			...
			for item in self.genesis_kitty.iter() {
				let mut new_price = (price + count) as u64;
				let kitty = Kitty::<T> {
					dna: *item,
					price: new_price,
					gender: Gender::Female,
					owner: self.owner.clone().unwrap(),
					create_date: self.genesis_date + count,
				};
				count +=1u64;
				Kitties::<T>::insert(item, kitty);
			}
			KittyId::<T>::put(self.genesis_kitty.len() as u32);
		}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn create_kitty(origin: OriginFor<T>, dna: Vec<u8>, price: u64) -> DispatchResult {
			...
			TotalKitty::<T>::insert(kitty.owner.clone(), amount_kitty);
		...
		#[pallet::weight(10_000 + T::DbWeight::get().writes(1).ref_time())]
		pub fn update_price(origin: OriginFor<T>, dna: T::Hash, price: u64) -> DispatchResult {
			let owner = ensure_signed(origin)?;
			let mut kitty = Kitties::<T>::get(&dna).ok_or(Error::<T>::NotKitty)?;
